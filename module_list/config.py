...
class config(Module):
    ...
    def update_config_value(self, name, value):
        ...
        try:
            ...
            last_value = getattr(self, name, config_info.default)
            ...
            if config_info.type:
                if config_info.type == int:
                    ...
                    if isinstance(value, str):
                        ...
                        try:
                            import json
                            value = json.loads(value.replace('\r','').replace('\n',''))
                        except:
                            # 如果是逗号分隔的，试图转成原列表对应的类型
                            if last_value is not None and len(last_value) > 0:
                                if isinstance(last_value[0],int):
                                    value = [int(v.strip()) for v in value.split(',') if v.strip()]  
                                elif isinstance(last_value[0],list) or isinstance(value[0],dict):
                                    value = [json.loads(v.strip()) for v in value.split(',') if v.strip()] 
                                else:
